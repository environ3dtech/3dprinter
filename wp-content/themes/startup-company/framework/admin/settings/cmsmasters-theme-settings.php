...
add_action('admin_menu', 'startup_company_add_menu');
add_action('admin_init', 'startup_company_register_settings');
...
function startup_company_get_settings() {
	$output = array();
	
	
	$page = startup_company_get_admin_page();
	...
	case 'cmsmasters-settings':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'startup-company' . CMSMASTERS_THEME_STYLE;
		$cmsmasters_settings_page_title = esc_html__('Theme General Settings', 'startup-company');
		$cmsmasters_page_sections = startup_company_options_general_sections();
		$cmsmasters_page_fields = startup_company_options_general_fields();
		$cmsmasters_page_tabs = startup_company_options_general_tabs();
	...
	case 'cmsmasters-settings' . '-font':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'startup-company' . CMSMASTERS_THEME_STYLE . '_font';
		$cmsmasters_settings_page_title = esc_html__('Theme Fonts Settings', 'startup-company');
		$cmsmasters_page_sections = startup_company_options_font_sections();
		$cmsmasters_page_fields = startup_company_options_font_fields();
		$cmsmasters_page_tabs = startup_company_options_font_tabs();
	...
	case 'cmsmasters-settings' . '-color':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'startup-company' . CMSMASTERS_THEME_STYLE . '_color';
		$cmsmasters_settings_page_title = esc_html__('Theme Color Schemes', 'startup-company');
		$cmsmasters_page_sections = startup_company_options_color_sections();
		$cmsmasters_page_fields = startup_company_options_color_fields();
		$cmsmasters_page_tabs = startup_company_options_color_tabs();
	...
	case 'cmsmasters-settings' . '-element':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'startup-company' . CMSMASTERS_THEME_STYLE . '_element';
		$cmsmasters_settings_page_title = esc_html__('Theme Elements Settings', 'startup-company');
		$cmsmasters_page_sections = startup_company_options_element_sections();
		$cmsmasters_page_fields = startup_company_options_element_fields();
		$cmsmasters_page_tabs = startup_company_options_element_tabs();
	...
	case 'cmsmasters-settings' . '-single':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'startup-company' . CMSMASTERS_THEME_STYLE . '_single';
		$cmsmasters_settings_page_title = esc_html__('Theme Single Posts Settings', 'startup-company');
		$cmsmasters_page_sections = startup_company_options_single_sections();
		$cmsmasters_page_fields = startup_company_options_single_fields();
		$cmsmasters_page_tabs = startup_company_options_single_tabs();
	...
	case 'cmsmasters-settings' . '-demo':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'startup-company' . CMSMASTERS_THEME_STYLE . '_demo';
		$cmsmasters_settings_page_title = esc_html__('Theme Settings Import/Export', 'startup-company');
		$cmsmasters_page_sections = startup_company_options_demo_sections();
		$cmsmasters_page_fields = startup_company_options_demo_fields();
		$cmsmasters_page_tabs = startup_company_options_demo_tabs();
...
function startup_company_create_settings_field($args = array()) {
	$defaults = array( 
		'id' => 		'default_field', 
		'title' => 		esc_html__('Default Field', 'startup-company'), 
		'desc' => 		esc_html__('This is a default description.', 'startup-company'), 
		'std' => 		'', 
		'type' => 		'text', 
...
function startup_company_register_settings() {
	$settings_output = startup_company_get_settings();
	...
	register_setting($cmsmasters_option_name, $cmsmasters_option_name, 'startup_company_validate_options');
	
	
	if (!empty($settings_output['cmsmasters_page_sections'])) {
		foreach ($settings_output['cmsmasters_page_sections'] as $id => $title) {
			add_settings_section($id, $title, 'startup_company_section_fn', __FILE__);
		}
	}
	...
	if (!empty($settings_output['cmsmasters_page_fields'])) {
		foreach ($settings_output['cmsmasters_page_fields'] as $option) {
			startup_company_create_settings_field($option);
		}
	}
...
function startup_company_settings_scripts() {
	wp_enqueue_style('wp-jquery-ui-dialog');
	
	
	wp_enqueue_style('startup-company-settings-css', get_template_directory_uri() . '/framework/admin/settings/css/cmsmasters-theme-settings.css', array(), '1.0.0', 'screen');
	
	
	if (is_rtl()) {
		wp_enqueue_style('startup-company-settings-css-rtl', get_template_directory_uri() . '/framework/admin/settings/css/cmsmasters-theme-settings-rtl.css', array(), '1.0.0', 'screen');
	}
	...
	wp_enqueue_script('startup-company-settings-js', get_template_directory_uri() . '/framework/admin/settings/js/cmsmasters-theme-settings.js', array('jquery', 'farbtastic'), '1.0.0', true);
	
	
	$theme_style_name = '';
	
	if (CMSMASTERS_THEME_STYLE_COMPATIBILITY) {
		foreach (startup_company_all_theme_styles() as $theme_style) {
	...
	wp_localize_script('startup-company-settings-js', 'cmsmasters_setting', array( 
		'palettes' => 						implode(',', cmsmasters_color_picker_palettes()), 
		'remove' => 						esc_attr__('Remove', 'startup-company'), 
		'remove_sidebar' => 				esc_html__('Do you realy want to remove this sidebar?', 'startup-company'), 
		'find' => 							esc_attr__('Find icons', 'startup-company'), 
		'remove_icon' => 					esc_html__('Do you realy want to remove this social icon?', 'startup-company'), 
		'theme_uri' => 						get_template_directory_uri(), 
		'shortname' => 						'startup-company', 
		'theme_style_name' => 				$theme_style_name, 
		'done' => 							esc_html__('All theme settings were imported successfully.', 'startup-company'), 
		'fail' => 							esc_html__("Error on page!!!\nPlease reload page and try again.", 'startup-company'), 
		'done_reset_settings' => 			esc_html__('Theme style settings were reset to default.', 'startup-company'), 
	...
	wp_enqueue_script('startup-company-settings-js-toggle', get_template_directory_uri() . '/framework/admin/settings/js/cmsmasters-theme-settings-toggle.js', array('jquery'), '1.0.0', true);
	
	wp_localize_script('startup-company-settings-js-toggle', 'cmsmasters_settings', array( 
		'shortname' => 	'startup-company' 
	));
...
function startup_company_add_menu() {
	$settings_output = startup_company_get_settings();
	...
	$add_menu_p( 
		esc_attr__('Theme Settings', 'startup-company'), 
		esc_attr__('Theme Settings', 'startup-company'), 
		'manage_options', 
		'cmsmasters-settings', 
		'startup_company_settings_page_fn', 
		'' 
	);
	...
	$cmsmasters_settings_general = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme General Settings', 'startup-company'), 
		esc_attr__('General', 'startup-company'), 
		'manage_options', 
		'cmsmasters-settings', 
		'startup_company_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_general, 'startup_company_settings_scripts');
	
	
	$cmsmasters_settings_font = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Fonts Settings', 'startup-company'), 
		esc_attr__('Fonts', 'startup-company'), 
		'manage_options', 
		'cmsmasters-settings' . '-font', 
		'startup_company_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_font, 'startup_company_settings_scripts');
	
	
	$cmsmasters_settings_color = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Color Schemes', 'startup-company'), 
		esc_attr__('Colors', 'startup-company'), 
		'manage_options', 
		'cmsmasters-settings' . '-color', 
		'startup_company_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_color, 'startup_company_settings_scripts');
	
	
	$cmsmasters_settings_element = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Elements Settings', 'startup-company'), 
		esc_attr__('Elements', 'startup-company'), 
		'manage_options', 
		'cmsmasters-settings' . '-element', 
		'startup_company_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_element, 'startup_company_settings_scripts');
	
	
	$cmsmasters_settings_single = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Single Posts Settings', 'startup-company'), 
		esc_attr__('Single Posts', 'startup-company'), 
		'manage_options', 
		'cmsmasters-settings' . '-single', 
		'startup_company_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_single, 'startup_company_settings_scripts');
	
	
	$cmsmasters_settings_demo = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Settings Import/Export', 'startup-company'), 
		esc_attr__('Import/Export', 'startup-company'), 
		'manage_options', 
		'cmsmasters-settings' . '-demo', 
		'startup_company_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_demo, 'startup_company_settings_scripts');
}


function startup_company_section_fn($desc) {
	$tab = startup_company_get_the_tab();
...
function startup_company_settings_page_fn() {
	$settings_output = startup_company_get_settings();
	$current_tab = startup_company_get_the_tab();
	
	echo '<div class="wrap">';
	
	startup_company_settings_page_header();
	...
	if ($current_tab != 'wpml') {
		echo '<form action="options.php" method="post" class="cmsmasters_admin_page"' . (($current_tab == 'recaptcha') ? ' style="background-color:#fdffc6; padding:35px 0 15px 25px; margin:0;"' : '') . '>' . 
			'<p class="submit-top">' . 
				(($current_tab != 'import' && $current_tab != 'export' && $current_tab != 'theme_style') ? '<input name="submit-top" type="submit" class="button button-primary button-large" value="' . esc_attr__('Save Changes', 'startup-company') . '" />' : '') . 
			'</p>';
			...
			echo '<p class="submit">';
				...
				if ($current_tab != 'import' && $current_tab != 'export' && $current_tab != 'theme_style') {
					echo '<input name="submit" type="submit" class="button button-primary button-large" value="' . esc_attr__('Save Changes', 'startup-company') . '" />';
				} elseif ($current_tab == 'import') {
					echo '<input name="import" type="button" class="button button-primary button-large cmsmasters-demo-import" value="' . esc_attr__('Import Settings', 'startup-company') . '" />';
				} elseif ($current_tab == 'theme_style') {
					echo '<input name="apply-theme-style" type="submit" class="button button-primary button-large cmsmasters-apply-theme-style" value="' . esc_attr__('Apply This Style', 'startup-company') . '" />';
					echo '<input name="reset-settings" type="button" class="button button-large cmsmasters-reset-settings" value="' . esc_attr__('Reset Theme Style Settings to Default', 'startup-company') . '" />';
				}
...
function startup_company_form_field_fn($args = array()) {
	extract($args);
	...
	case 'select_theme_style':
		$theme_style_option = get_option('cmsmasters_startup-company_theme_style');
		
		echo '<select id="' . $id . '" class="select' . $field_class . '" name="cmsmasters_startup-company_theme_style">';
	...
	case 'typorgaphy':
		...
		if ($system_font) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				foreach (startup_company_system_fonts_list() as $key => $value) {
					echo '<option value="' . esc_attr($key) . '"' . (($options[$id . '_system_font'] == $key) ? ' selected="selected"' : '') . '>' . esc_html($value) .'</option>';
				}
				
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_system_font">' . esc_html__('System Font', 'startup-company') . '</label>' . 
			'</div>';
		}
		...
		if ($google_font) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_google_font">' . esc_html__('Google Font', 'startup-company') . '</label>' . 
			'</div>';
		}
		...
		if ($font_size) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				' &nbsp; ' . 
				'<label for="' . $id . '_font_size_number">' . esc_html__('Font Size', 'startup-company') . ' <em>(' . esc_html__('in pixels', 'startup-company') . ')</em></label>' . 
			'</div>';
		}
		...
		if ($line_height) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				' &nbsp; ' . 
				'<label for="' . $id . '_line_height_number">' . esc_html__('Line Height', 'startup-company') . ' <em>(' . esc_html__('in pixels', 'startup-company') . ')</em></label>' . 
			'</div>';
		}
		...
		if ($font_weight) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_font_weight">' . esc_html__('Font Weight', 'startup-company') . '</label>' . 
			'</div>';
		}
		...
		if ($font_style) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_font_style">' . esc_html__('Font Style', 'startup-company') . '</label>' . 
			'</div>';
		}
		...
		if ($text_transform) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_text_transform">' . esc_html__('Text Transform', 'startup-company') . '</label>' . 
			'</div>';
		}
		...
		if ($text_decoration) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				foreach (startup_company_text_decoration_list() as $key => $value) {
					echo '<option value="' . esc_attr($key) . '"' . (($options[$id . '_text_decoration'] == $key) ? ' selected="selected"' : '') . '>' . esc_html($value) .'</option>';
				}
				
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_text_decoration">' . esc_html__('Text Decoration', 'startup-company') . '</label>' . 
			'</div>';
		}
	...
	case 'sidebar':
		...
		'<div class="sidebar_management">' . 
			'<p>' . 
				...
				'<input class="button" type="button" id="add_sidebar" value="' . esc_attr__('Add Sidebar', 'startup-company') . '" />' . 
			'</p>' . 
			'<div></div>' . 
			'<ul>';
	...
	case 'social':
		...
		'<div class="icon_management">' . 
			'<p>' . 
				...
				'<input id="' . $id . '_button" class="cmsmasters_icon_choose_button button" type="button" value="' . esc_attr__('Choose icon', 'startup-company') . '" />' . 
				'<a href="#" class="cmsmasters_remove_icon admin-icon-remove" title="' . esc_attr__('Cancel changes', 'startup-company') . '"></a>' . 
			'</p>' . 
			'<span class="cl"><br /></span>' . 
			'<span class="icon_upload_link" style="display:none;">' . 
				'<label for="new_icon_color">' . 
					'<span class="cmsmasters_col_label">' . esc_html__('Icon Color', 'startup-company') . '</span>' . 
				...
				'<label for="new_icon_hover">' . 
					'<span class="cmsmasters_col_label">' . esc_html__('Icon Hover Color', 'startup-company') . '<br /><em>' . esc_html__('if not selected - Icon Color will be used', 'startup-company') . '</em></span>' . 
				...
				'<label for="new_icon_link">' . 
					...
					esc_html__('Icon Link', 'startup-company') . 
				'</label>' . 
				'<label for="new_icon_title">' . 
					'<input class="all-options" type="text" id="new_icon_title" /> ' . 
					esc_html__('Icon Title', 'startup-company') . 
				'</label>' . 
				'<label for="new_icon_target">' . 
					'<input type="checkbox" id="new_icon_target" value="true" /> ' . 
					esc_html__('Open link in a new tab/window?', 'startup-company') . 
				'</label>' . 
			'</span>' . 
			'<span class="cl"></span>' . 
			'<input class="button button-primary" type="button" id="add_icon" value="' . esc_attr__('Add Icon', 'startup-company') . '" />' . 
			'<input class="button button-primary" type="button" id="edit_icon" value="' . esc_attr__('Save Icon', 'startup-company') . '" />' . 
			'<ul>';
			...
			if (isset($options[$id]) && is_array($options[$id])) {
				foreach($options[$id] as $icon) {
					...
					echo '<li>' . 
						...
						'</div>' . 
						'<a href="#" class="icon_del admin-icon-remove" title="' . esc_attr__('Remove', 'startup-company') . '"></a> ' . 
...
function startup_company_validate_options($input) {
	$valid_input = array();
	...
	foreach ($options as $option) {
		...
		case 'text':
			...
			case 'numeric':
				...
				$valid_input[$option['id']] = (is_numeric($input[$option['id']])) ? $input[$option['id']] : esc_html__('Expecting a Numeric value!', 'startup-company');
				
				if (is_numeric($input[$option['id']]) == false) {
					add_settings_error(
						$option['id'],
						'startup-company' . '_txt_numeric_error',
						esc_html__('Expecting a Numeric value! Please fix.', 'startup-company'),
						'error'
					);
				}
			...
			case 'multinumeric':
				...
				if ($input[$option['id']] != '') {
					$valid_input[$option['id']] = (preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) == 1) ? $input[$option['id']] : esc_html__('Expecting comma separated numeric values', 'startup-company');
				} else {
				...
				if ($input[$option['id']] != '' && preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) != 1) {
					add_settings_error(
						$option['id'],
						'startup-company' . '_txt_multinumeric_error',
						esc_html__('Expecting comma separated numeric values! Please fix.','startup-company'),
						'error'
					);
				}
			...
			case 'email':
				...
				if ($input[$option['id']] != '') {
					$valid_input[$option['id']] = (is_email($input[$option['id']]) !== false) ? $input[$option['id']] : esc_html__('Invalid email! Please re-enter!', 'startup-company');
				} elseif ($input[$option['id']] == '') {
					$valid_input[$option['id']] = esc_html__('This setting field cannot be empty! Please enter a valid email address.', 'startup-company');
				}
				
				if (is_email($input[$option['id']]) == false || $input[$option['id']] == '') {
					add_settings_error(
						$option['id'],
						'startup-company' . '_txt_email_error',
						esc_html__('Please enter a valid email address.', 'startup-company'),
						'error'
					);
				}
		...
		case 'number':
			$input[$option['id']] = trim($input[$option['id']]);
			$valid_input[$option['id']] = (is_numeric($input[$option['id']])) ? $input[$option['id']] : esc_html__('Number!', 'startup-company');
...
function startup_company_show_msg($message, $msgclass = 'info') {
	echo '<div id="message" class="' . $msgclass . '">' . $message . '</div>';
}


function startup_company_admin_msgs() {
	global $pagenow;
...
function startup_company_add_global_options($reset = false) {
	...
	foreach (startup_company_all_color_schemes_list() as $key => $value) {
		array_push($cmsmasters_option_names, array( 
			'cmsmasters_options_' . 'startup-company' . CMSMASTERS_THEME_STYLE . '_color_' . $key, 
			startup_company_options_color_fields($key) 
		));
	}
		...
		if ($cmsmasters_option_name[1] !== '') {
			foreach ($cmsmasters_option_name[1] as $selected_option) {
				if ( 
					...
					$selected_option['id'] !== 'startup-company' . '_social_icons' 
				) {
...
function startup_company_get_global_options() {
	...
	foreach (startup_company_all_color_schemes_list() as $key => $value) {
		array_push($cmsmasters_option_names, 'cmsmasters_options_' . 'startup-company' . CMSMASTERS_THEME_STYLE . '_color_' . $key);
	}
