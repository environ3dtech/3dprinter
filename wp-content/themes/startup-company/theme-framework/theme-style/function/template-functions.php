...
/* Post Type Video */
function startup_company_post_type_video($cmsmasters_id, $video_type = '', $video_link = '', $video_links = '', $size = 'post-thumbnail') {
	if (!post_password_required()) {
		if ($video_type == 'selfhosted' && !empty($video_links) && sizeof($video_links) > 0) {
			...
			startup_company_thumb($cmsmasters_id, $size, true, false, true, false, true, true, false);
		}
	}
}
...
/* Post Type Slider */
function startup_company_post_type_slider($cmsmasters_id, $images, $size = 'post-thumbnail', $slider_data = '') {
	if (!post_password_required()) {
		if (sizeof($images) > 1) {
			echo '<div' . 
			...
			startup_company_thumb($cmsmasters_id, $size, false, 'img_' . $cmsmasters_id, true, true, true, true, $images[0]);
		} elseif (has_post_thumbnail()) {
			startup_company_thumb($cmsmasters_id, $size, false, 'img_' . $cmsmasters_id, true, true, true, true, false);
		}
	}
}
...
/* Post Type Gallery */
function startup_company_post_type_gallery($cmsmasters_id, $images, $columns, $size_col = 'post-thumbnail', $size_full = 'post-thumbnail') {
	if (!post_password_required()) {
		...
		if (sizeof($images) > 0 && $images[0] != '') {
			...
			startup_company_thumb($cmsmasters_id, $size_full, false, 'img_' . $cmsmasters_id, true, true, false, true, false);
		}
	}
}
...
/* Post Type Image */
function startup_company_post_type_image($cmsmasters_id, $image_link = '', $size = 'cmsmasters-masonry-thumb') {
	if (!post_password_required()) {
		if ($image_link != '') {
			startup_company_thumb($cmsmasters_id, $size, false, 'img_' . $cmsmasters_id, false, false, false, true, $image_link);
		} elseif (has_post_thumbnail()) {
			startup_company_thumb($cmsmasters_id, $size, false, 'img_' . $cmsmasters_id, false, false, false, true, false);
		}
	}
}
...
/* Post Type Audio */
function startup_company_post_type_audio($audio_links) {
...
function startup_company_get_header_search_form($cmsmasters_option) {
	if (
		...
		$out_form = "<form method=\"get\" action=\"" . esc_url(home_url('/')) . "\">
			<div class=\"cmsmasters_header_search_form_field\">
				<input type=\"search\" name=\"s\" placeholder=\"" . esc_attr__('Enter Keywords', 'startup-company') . "\" value=\"\" />
				<button type=\"submit\">Search</button>
			</div>
		</form>";
		
		
		$out_form = apply_filters('startup_company_get_header_search_form_filter', $out_form);
...
/* Theme Error Styles */
function startup_company_theme_error_styles() {
	$out = "";
	...
	if (is_404()) {
		...
		if (
			$cmsmasters_option['startup-company' . '_error_bg_img_enable'] && 
			$cmsmasters_option['startup-company' . '_error_bg_image'] != ''
		) {
			$error_bg_image = explode('|', $cmsmasters_option['startup-company' . '_error_bg_image']);
			...
			$out .= "
	.error .error_bg {
		background-image : " . (!empty($cmsmasters_option['startup-company' . '_error_bg_image']) ? 'url(' . ((is_numeric($error_bg_image[0])) ? $error_bg_image_url[0] : $error_bg_image[1]) . ')' : 'none') . ";
		background-position : " . (!empty($cmsmasters_option['startup-company' . '_error_bg_pos']) ? $cmsmasters_option['startup-company' . '_error_bg_pos'] : 'top center') . ";
		background-repeat : " . (!empty($cmsmasters_option['startup-company' . '_error_bg_rep']) ? $cmsmasters_option['startup-company' . '_error_bg_rep'] : 'repeat') . ";
		background-attachment : " . (!empty($cmsmasters_option['startup-company' . '_error_bg_att']) ? $cmsmasters_option['startup-company' . '_error_bg_att'] : 'scroll') . ";
		background-size : " . (!empty($cmsmasters_option['startup-company' . '_error_bg_size']) ? $cmsmasters_option['startup-company' . '_error_bg_size'] : 'auto') . ";
	}
";
		}
		...
		$out .= "
	.error .error_bg {
		" . cmsmasters_color_css('background-color', $cmsmasters_option['startup-company' . '_error_bg_color']) . "
	}
	
	.error .error_title, 
	.error .error_subtitle {
		" . cmsmasters_color_css('color', $cmsmasters_option['startup-company' . '_error_color']) . "
	}
";
	}
...
add_action('wp_enqueue_scripts', 'startup_company_theme_error_styles');



/* Get Page Heading Function */
function startup_company_page_heading() {
	if (is_404() || is_home()) {
		...
		if (
			...
			$cmsmasters_header_overlaps = $cmsmasters_option['startup-company' . '_header_overlaps'] ? 'true' : 'false';
			
			$cmsmasters_heading_alignment = $cmsmasters_option['startup-company' . '_heading_alignment'];
			$cmsmasters_heading_scheme = $cmsmasters_option['startup-company' . '_heading_scheme'];
			
			$cmsmasters_breadcrumbs = $cmsmasters_option['startup-company' . '_breadcrumbs'] ? 'true' : 'false';
		}
				...
				if ($cmsmasters_heading == 'custom') {
					...
					if (is_search()) {
						...
						if (!empty($wp_query->found_posts)) {
							echo sprintf(esc_html(_n('1 search result for: %2$s', '%1$d search results for: %2$s', $wp_query->found_posts, 'startup-company')), $wp_query->found_posts, get_search_query());
						} else {
							echo sprintf(esc_html__('0 search results for: %s', 'startup-company'), get_search_query());
						}
					} elseif (is_archive()) {
						if (is_author()) {
							if (get_the_author_meta('first_name') != '' || get_the_author_meta('last_name') != '') {
								echo sprintf(esc_html__('Author: %1$s (%2$s %3$s)', 'startup-company'), '<span class="vcard">' . get_the_author() . '</span>', get_the_author_meta('first_name'), get_the_author_meta('last_name'));
							} else {
								echo sprintf(esc_html__('Author: %s', 'startup-company'), '<span class="vcard">' . get_the_author() . '</span>');
							}
						} else {
...
function startup_company_theme_heading_styles() {
	$cmsmasters_option = startup_company_get_global_options();
	...
	if (
		...
		$cmsmasters_heading_height = $cmsmasters_option['startup-company' . '_heading_height'];
		$cmsmasters_heading_bg_color = $cmsmasters_option['startup-company' . '_heading_bg_color'];
		$cmsmasters_heading_bg_img_enable = $cmsmasters_option['startup-company' . '_heading_bg_image_enable'] ? 'true' : 'false';
		$cmsmasters_heading_bg_img = $cmsmasters_option['startup-company' . '_heading_bg_image'];
		$cmsmasters_heading_bg_rep = $cmsmasters_option['startup-company' . '_heading_bg_repeat'];
		$cmsmasters_heading_bg_att = $cmsmasters_option['startup-company' . '_heading_bg_attachment'];
		$cmsmasters_heading_bg_size = $cmsmasters_option['startup-company' . '_heading_bg_size'];
	}
...
add_action('wp_enqueue_scripts', 'startup_company_theme_heading_styles');



/* Get Social Icons Styles Function */
function startup_company_theme_social_icons_styles() {
...
add_action('wp_enqueue_scripts', 'startup_company_theme_social_icons_styles');



/* Get Posts Thumbnail Function */
function startup_company_thumb($cmsmasters_id, $type = 'post-thumbnail', $link = true, $group = false, $preload = true, $highImg = false, $fullwidth = true, $show = true, $attachment = false, $unique = false, $link_icon = false, $placeholder_icon = 'cmsmasters_theme_icon_image') {
	$args = array( 
...
function startup_company_thumb_rollover($cmsmasters_id, $type = 'post-thumbnail', $rollover = true, $open_link = true, $group = false, $attachment_images = false, $attachment_video_type = false, $attachment_video_link = false, $attachment_video_links = false, $highImg = false, $show = true, $link_redirect = false, $link_url = false, $link_target = false, $placeholder_icon = 'cmsmasters_theme_icon_image', $puzzle_image = '') {
	...
	if ($rollover) {
		...
		$out .= '<div class="cmsmasters_img_rollover">' . 
			'<a href="' . (($link_redirect == 'true' && $link_url != '') ? esc_url($link_url) : esc_url(get_permalink($cmsmasters_id))) . '"' . (($link_redirect == 'true' && $link_target == 'true') ? ' target="_blank"' : '') . ' title="' . esc_attr($cmsmasters_title) . '" class="cmsmasters_open_post_link button">' . esc_html__('Read more', 'startup-company') . '</a>' . 
		'</div>';
	} else {
		$out .= '<div class="cmsmasters_img_rollover">' . 
...
function startup_company_thumb_small($cmsmasters_id, $type = 'post', $w = 100, $h = 100, $show = true) {
...
function startup_company_theme_category_styles() {
	$out = "";
	
	
	if (CMSMASTERS_COLORED_CATEGORIES) {
		$args = startup_company_colored_categories_args();
...
add_action('wp_enqueue_scripts', 'startup_company_theme_category_styles');



/* Get Page Container Classes */
function startup_company_get_page_classes($cmsmasters_option, $classes = false) {
	$browser = new Browser();
	...
	echo 'cmsmasters_' . $cmsmasters_option['startup-company' . '_theme_layout'] . ' ';
	...
	if ($cmsmasters_option['startup-company' . '_fixed_header']) {
		echo 'fixed_header ';
	}
	
	
	if ($cmsmasters_option['startup-company' . '_header_top_line']) {
	...
	if ($cmsmasters_option['startup-company' . '_header_styles'] == 'r_nav') {
	...
	if ($cmsmasters_header_overlaps == '') {
		$cmsmasters_header_overlaps = $cmsmasters_option['startup-company' . '_header_overlaps'];
	}
...
/* Theme Header Styles */
function startup_company_theme_header_styles() {
	$cmsmasters_option = startup_company_get_global_options();
	
	$defaults = startup_company_settings_general_defaults();
	
	
	$header_top_height = (($cmsmasters_option['startup-company' . '_header_top_height'] !== '') ? $cmsmasters_option['startup-company' . '_header_top_height'] : $defaults[$tab]['startup-company' . '_header_top_height']);
	...
	$header_bot_height = (($cmsmasters_option['startup-company' . '_header_bot_height'] !== '') ? $cmsmasters_option['startup-company' . '_header_bot_height'] : $defaults[$tab]['startup-company' . '_header_bot_height']);
...
";
	...
	if ($cmsmasters_option['startup-company' . '_custom_css'] != '') {
		$out .= stripslashes($cmsmasters_option['startup-company' . '_custom_css']);
	}
	
	wp_add_inline_style('startup-company-style', $out);
}

add_action('wp_enqueue_scripts', 'startup_company_theme_header_styles');
