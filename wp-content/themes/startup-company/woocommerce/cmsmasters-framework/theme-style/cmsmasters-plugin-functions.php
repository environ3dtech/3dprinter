...
function startup_company_woocommerce_register_styles_scripts() {
	// Styles
	wp_enqueue_style('startup-company-woocommerce-style', get_template_directory_uri() . '/woocommerce/cmsmasters-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/css/plugin-style.css', array(), '1.0.0', 'screen');
	
	wp_enqueue_style('startup-company-woocommerce-adaptive', get_template_directory_uri() . '/woocommerce/cmsmasters-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/css/plugin-adaptive.css', array(), '1.0.0', 'screen');
	
	
	if (is_rtl()) {
		wp_enqueue_style('startup-company-woocommerce-rtl', get_template_directory_uri() . '/woocommerce/cmsmasters-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/css/plugin-rtl.css', array(), '1.0.0', 'screen');
	}
	
	
	// Scripts
	wp_enqueue_script('startup-company-woocommerce-script', get_template_directory_uri() . '/woocommerce/cmsmasters-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/js/jquery.plugin-script.js', array('jquery'), '1.0.0', true);
	...
	wp_localize_script('startup-company-woocommerce-script', 'cmsmasters_woo_script', array( 
...
add_action('wp_enqueue_scripts', 'startup_company_woocommerce_register_styles_scripts');


/* Register Default Woocommerce Theme Sidebar */
function startup_company_woocommerce_the_widgets_init() {
	...
	register_sidebar(
		array(
			'name' => esc_html__('Shop Sidebar', 'startup-company'), 
			'id' => 'sidebar_shop', 
			'description' => esc_html__('Widgets in this area will be shown in left or right sidebar on shop page.', 'startup-company'), 
...
add_action('widgets_init', 'startup_company_woocommerce_the_widgets_init');
